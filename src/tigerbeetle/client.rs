...
impl AppTbClient {
    ...
    pub async fn lookup_accounts(&self, ids: Vec<u128>) -> Result<Vec<Account>, String> {
        ...
        self.sender
            .send(TbCommand::LookupAccounts { ids, response: tx })
            .await
            .map_err(|_| "Failed to send LookupAccounts command to TB worker".to_string())?;

        rx.await
            .map_err(|_| "Worker dropped the oneshot channel".to_string())?
    }
    ...
    pub async fn get_account_balances(
        &self,
        ids: Vec<u128>,
    ) -> Result<Vec<AppAccountBalance>, String> {
        let (tx, rx) = oneshot::channel();

        self.sender
            .send(TbCommand::GetAccountBalances { ids, responder: tx })
            .await
