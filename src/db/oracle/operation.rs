    ...
    pub async fn get_integration_operation(
        ...
        self.pool.with_connection(move |connection| {
            ...
            let mut rows = connection.query(
                "SELECT o.outbox_event_id,o.event_type,o.aggregate_type,o.aggregate_id,o.status,o.attempt_count,TO_CHAR(SYS_EXTRACT_UTC(o.created_at),'YYYY-MM-DD\"T\"HH24:MI:SS.FF3\"Z\"'),TO_CHAR(SYS_EXTRACT_UTC(o.published_at),'YYYY-MM-DD\"T\"HH24:MI:SS.FF3\"Z\"'),TO_CHAR(SYS_EXTRACT_UTC(r.materialized_at),'YYYY-MM-DD\"T\"HH24:MI:SS.FF3\"Z\"') FROM integration_outbox o LEFT JOIN runtime_materialization_receipts r ON r.operation_id=o.operation_id WHERE o.operation_id=:1",
                &[&raw],
            ...
            let outbox_status: String = row.get(4).map_err(map_error)?;
            let materialized_at: Option<String> = row.get(8).map_err(map_error)?;
            let status = if materialized_at.is_some() { IntegrationOperationStatus::Materialized } else { match outbox_status.as_str() { "PENDING" => IntegrationOperationStatus::Pending, "PUBLISHING" => IntegrationOperationStatus::Publishing, "PUBLISHED" => IntegrationOperationStatus::Published, "DEAD_LETTER" => IntegrationOperationStatus::DeadLetter, _ => return Err(DbError::Query("unknown integration outbox status".to_string())) } };
